...
public class ColorGenerator 
{
    ...
    {
        settings.planetMaterial.SetVector("_elevationMinMax", new Vector4(elevationMixMax.Min, elevationMixMax.Max));
    }
    ...
    public void UpdateColors()
    {
        ...
        foreach (var biome in settings.biomeColorSettings.biomes)
        {
            for (int i = 0; i < textureResolution * 2; i++)
            {
                ...
                {
                    gradientCol = settings.oceanGradient.Evaluate(i / (textureResolution - 1f));
                }
                else
                {
